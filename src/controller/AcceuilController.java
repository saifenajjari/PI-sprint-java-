...
//For multithreading: Create executor that uses daemon threads:
    ...
    void btn_afficher(MouseEvent event) {
        if (event.getTarget() == id_ajouter_enfant) {
            try {
                AnchorPane newLoadedPane = FXMLLoader.load(getClass().getResource("/view/EnfantView.fxml"));
   ...
   {
       try {
           AnchorPane newLoadedPane = FXMLLoader.load(getClass().getResource("/views/ListeJardinView.fxml"));
           content.getChildren().clear();
           content.getChildren().add(newLoadedPane);
           content.setPrefWidth(900.0);
       } catch (IOException ex) {
